...
class OutputGenerator:
    ...
    def _collect_stats(self, all_files: List[FileInfo], filtered_files: List[FileInfo], 
        ...
        stats = {
            ...
            'database_files': len([c for c in file_contents.values() if c.database_info]),
    ...
    def _generate_markdown_output(self, project_name: str, root_path: Path, stats: Dict[str, Any], 
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            if self.include_file_stats:
                ...
                f.write(f"- **Adatbázis fájlok:** {stats['database_files']}\n")
            ...
            for file_path, file_content in sorted_contents:
                ...
                if file_content.error:
                    ...
                    if file_content.database_info:
                        f.write(f"**Adatbázis típusa:** {file_content.database_info.db_type}\n")
                        if file_content.database_info.version:
                            f.write(f"**Verzió:** {file_content.database_info.version}\n")
                        if file_content.database_info.table_count is not None:
                            f.write(f"**Táblák száma:** {file_content.database_info.table_count}\n")
                        f.write("\n")
                        if file_content.database_info.schema_info:
                            f.write(f"**Séma információ:**\n")
                            f.write("```sql\n")
                            f.write(file_content.database_info.schema_info)
                            f.write("\n```\n\n")
                    elif file_content.content:
                        f.write(f"{file_content.content}\n\n")
                    else:
                        f.write("[Bináris fájl - nincs további információ]\n\n")
                else:
